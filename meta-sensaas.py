...
#execute ./<>.py sdf_file_target sdf_file_source -r nb -s score_type -l x
# -r ; number_of_repeats (used only on first molecule target and first molecule source sdf)
# -s ; score_type { source target mean }
# -l ; loop over x in case of database mode (not used with -r nb)
...
try:
    ...
    print('usage: meta-sensaas.py sdf-target-file-name sdf-source-file-name -r nb -s score_type -l x\noption -r: nb is an integer\noption -s: { source target mean }\noption -l: x is an integer')
    quit()
            ...
            while(loopi < loop):
                ...
                if(loopi > 0):
                    if(len(tabscore[0])<7):
                        ...
                        sgh=float(tabscore[0][7])
                        sgh='%.3f' % (sgh)
                        sg=float(tabscore[0][1])
                        sg='%.3f' % (sg)
                        sh=float(tabscore[0][5])
                        sh='%.3f' % (sh)
                        ligneloop=[]
                        solfile=open('Source_tran.sdf', 'r')
                        ligneloop=solfile.readlines()
                        solfile.close()
                else:
                    sensaasfail=0
                    if(len(tabscore[0])<7):
                ...
                if((score_type=="mean" or score_type=="target") and sensaasfail==0):
                    ...
                    #scores of target
                    if(loopi > 0):
                        tgh=float(tabscore[0][7])
                        tgh='%.3f' % (tgh)
                        tg=float(tabscore[0][1])
                        tg='%.3f' % (tg)
                        th=float(tabscore[0][5])
                        th='%.3f' % (th)
                    else:
                        scoregfithfit_target.append(tabscore[0][7])
                        scoregfit_target.append(tabscore[0][1])
                    ...
                    #mean
                    if(loopi > 0):
                        mgh=(float(sgh)+float(tgh))/2
                        mgh='%.3f' % (mgh)
                        mg=(float(sg)+float(tg))/2
                        mg='%.3f' % (mg)
                        mh=(float(sh)+float(th))/2
                        mh='%.3f' % (mh)
                    else:
                        mgh=(float(scoregfithfit[k])+float(scoregfithfit_target[k]))/2
                        mgh='%.3f' % (mgh)
                    ...
                    if(score_type=="target"):
                        if(loopi > 0):
                            if(float(scoregfithfit_target[k]) < float(tgh)):
                                scoregfithfit_target[k]=tgh
                                scoregfit_target[k]=tg
                                scorehfit_target[k]=th
                                lignesol=[]
                                lignesol=ligneloop
                        ...
                        if(loopi > 0):
                            if(float(scoregfithfit_mean[k]) < float(mgh)):
                                scoregfithfit_mean[k]=mgh
                                scoregfit_mean[k]=mg
                                scorehfit_mean[k]=mh
                                lignesol=[]
                                lignesol=ligneloop
                    ...
                    if(loopi > 0):
                        if(float(scoregfithfit[k]) < float(sgh)):
                            scoregfithfit[k]=sgh
                            scoregfit[k]=sg
                            scorehfit[k]=sh
                            lignesol=[]

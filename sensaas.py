...
# pdb = PDB format file (only reads ATOM or HETATM lines with coordinates; not appropriate for aminoacids/protein !)
# dot = PDB format file where ATOM or HETATM lines contain coordinates of dots and the atom element defining the label
# pcd = PCD format file (used in 3D data processing such as Open3d)
# xyzrgb = xyzrgb format file (ascii file used in 3D data processing such as Open3d; contains coordinates of dots and its color)
#
...
if (targettype=="sdf"):
    pcdxyz,pcdrgb,pcd1xyz,pcd1rgb,pcd2xyz,pcd2rgb,pcd3xyz,pcd3rgb,pcd4xyz,pcd4rgb = molsurface("sdf",target,TemplateDots,nbDots,0)
elif (targettype=="pdb"):
    pcdxyz,pcdrgb,pcd1xyz,pcd1rgb,pcd2xyz,pcd2rgb,pcd3xyz,pcd3rgb,pcd4xyz,pcd4rgb = molsurface("pdb",target,TemplateDots,nbDots,0)
elif (targettype=="dot"):
    read_pdbdot(target)
    #os.rename("dots.xyzrgb","Target.xyzrgb")
    shutil.copyfile("dots.xyzrgb","Target.xyzrgb")
...
if (sourcetype=="sdf"):
    pcdxyz,pcdrgb,pcd1xyz,pcd1rgb,pcd2xyz,pcd2rgb,pcd3xyz,pcd3rgb,pcd4xyz,pcd4rgb = molsurface("sdf",source,TemplateDots,nbDots,0)
elif (sourcetype=="pdb"):
    pcdxyz,pcdrgb,pcd1xyz,pcd1rgb,pcd2xyz,pcd2rgb,pcd3xyz,pcd3rgb,pcd4xyz,pcd4rgb = molsurface("pdb",source,TemplateDots,nbDots,0)
elif (sourcetype=="dot"):
    read_pdbdot(source)
    #os.rename("dots.xyzrgb","Source.xyzrgb")
    shutil.copyfile("dots.xyzrgb","Source.xyzrgb")
    os.remove("dots.xyzrgb")
